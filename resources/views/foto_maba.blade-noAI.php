...
<head>
    ...
    <style>
        ...
        .results-preview { height: 190px; border: 2px dashed #cbd5e1; border-radius: 12px; display: flex; align-items: center; justify-content: center; background: #fff; overflow: hidden; position: relative; }
...
<script>
        ...
        fetch(`{{ route('foto.cari') }}?nim=${nim}`)
            ...
            .then(data => {
                if(data.success) {
                    ...
                    const existingPhotoUrl = `{{ asset('storage/foto_maba') }}/${nim}.jpg?v=${cacheBuster}`;
                    ...
                    resultsBox.innerHTML = `
                        <div class="position-relative w-100 h-100 d-flex justify-content-center align-items-center bg-light">
                            <span class="badge bg-warning text-dark position-absolute top-0 start-50 translate-middle-x mt-2" style="font-size:0.65rem; z-index:5;">FOTO TERDAHULU</span>
                    ...
                    bc.postMessage({ type: 'verifikasi', nama: data.nama, gender: data.gender });
                } else {
    ...
    function takeSnap() {
        ...
        .then(data => {
            ...
            bc.postMessage({ type: 'tersimpan', nim: nimInput.value });
