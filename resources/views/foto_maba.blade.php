...
<head>
    ...
    <style>
        ...
        .shoulder-line {
            ...
            width: 80%;
            height: 30%;
            border-top: 4px dashed rgba(52, 211, 153, 0.6);
...
<body>
    ...
    <div class="app-container">
        ...
        <div class="row g-4">
            ...
            <div class="col-md-7">
                <div class="card mb-4">
                    <div class="card-header" style="background-color: #f1f5f9; color: #334155; font-weight: 800;">
                        VERIFIKASI MAHASISWA</div>
                    <div class="card-body p-4">
                        <div class="input-group input-group-lg mb-3 shadow-sm">
                            <input type="text" id="nim" class="form-control border-primary"
                                placeholder="Scan Barcode / Ketik NIM..." autofocus>
                ...
                <div class="card">
                    <div class="card-body p-4">
                        <div class="row align-items-center">
                            ...
                            <div class="col-7">
                                <div id="log-status" class="mb-3 text-center small fw-bold text-uppercase"
                                    style="letter-spacing: 1px;">STANDBY</div>

                                <div class="d-grid gap-3">
                                    <button id="btn-capture" class="btn btn-danger btn-lg py-3 fw-bold shadow"
                                        onclick="takeSnap()" disabled>
                                        📸 AMBIL FOTO (SPASI)
                                    </button>
                                    <button class="btn btn-outline-secondary btn-sm rounded-pill"
                                        onclick="resetForm()">BATAL / REFRESH</button>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        function cariData() {
            ...
            // prepare token untuk security call internal API
            const csrfToken = document.head.querySelector("[name~=csrf-token][content]").content;
            fetch("{{ route('get.mhs.detail') }}", {
                    method: 'post',
                    body: JSON.stringify({
                        nim: nim
                    }),
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        document.getElementById('nama-val').innerText = data.detail_mhs.nama;
                        document.getElementById('prodi-val').innerText = data.detail_mhs.prodi;
                        ...
                        // ==========================================
                        // Mengambil waktu saat ini (dalam milidetik) sebagai parameter unik
                        const cacheBuster = new Date().getTime();

                        // PERBAIKAN: Mengambil foto secara langsung dari server pusat SINAU
                        const existingPhotoUrl =
                            `https://sinau.uinsa.ac.id/uploads/fotomhs/${nim}.jpg?v=${cacheBuster}`;

                        // Coba tampilkan foto lama. Jika file tidak ada (onerror), tampilkan teks "Belum Ada Foto"
                        ...
                        <div class="position-relative w-100 h-100 d-flex justify-content-center align-items-center bg-light">
                            ...
                            <img src="${existingPhotoUrl}" class="img-fluid h-100 w-200" style="object-fit: cover;" 
                    ...
                    `;
                        ...
                        bc.postMessage({
                            type: 'verifikasi',
                            nama: data.detail_mhs.nama,
                            gender: data.detail_mhs.gender
                        });
                    } else {
                    ...
                    return
                    // next fetch data mahasiswa dari api
                    // old route `{{ route('foto.cari') }}?nim=${nim}`
        ...
        function takeSnap() {
            ...
            // Data base64 ini kebal terhadap cache browser
            const dataURL = canvas.toDataURL('image/jpeg', 0.95);

            // Menampilkan hasil jepretan BARU seketika di layar
            resultsBox.innerHTML = `
            <div class="position-relative h-100 w-200">
                ...
                <img src="${dataURL}" class="img-fluid h-100 w-200" style="object-fit: cover;">
            </div>
        `;

            // Proses unggah ke server lokal
            fetch('{{ route('foto.simpan') }}', {
                ...
                .then(data => {
                    ...
                    // Auto-reset setelah 2 detik agar siap untuk mahasiswa berikutnya
                    setTimeout(() => {
                        resetForm();
                    }, 2000);
                });
